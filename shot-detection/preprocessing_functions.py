...
def frames_to_float(directory, root_path, image_tmpl, color):
    '''converts all frames to a np.array with float numbers.'''
    fr_list = []
    fr_amount = len(next(os.walk(root_path))[2]) #counts the amount of frames
...
def frames_to_tensor(directory, root_path, image_tmpl, import_idx):
